...
class MetricsMixin:
    ...
    def global_metrics_by_field(
        ...
        def compute_age(records, indicators):
            indicators = indicators.copy()
            indicators = indicators.assign(
                age=(records.year.max() - indicators.first_publication_year + 1).astype(
                    int
                )
            )
            return indicators
        ...
        # -------------------------------------------------------------------------------
        def remove_user_stopwords_from_items(indicators):
            indicators = indicators.copy()
            stopwords = self.load_user_stopwords()
            indicators = indicators.drop(stopwords, axis=0, errors="ignore")

            return indicators

        indicators = remove_user_stopwords_from_items(indicators)

        # -------------------------------------------------------------------------------
        indicators = indicators.drop(field, axis=1)

        # -------------------------------------------------------------------------------
        def compute_rank_gcs(indicators):
            indicators = indicators.copy()
            indicators = self.sort_indicators_by_metric(indicators, "global_citations")
            indicators.insert(0, "rank_gcs", range(1, len(indicators) + 1))
            return indicators

        indicators = compute_rank_gcs(indicators)

        # -------------------------------------------------------------------------------
        def compute_rank_lcs(indicators):
            indicators = indicators.copy()
            indicators = self.sort_indicators_by_metric(indicators, "local_citations")
            indicators.insert(0, "rank_lcs", range(1, len(indicators) + 1))
            return indicators

        indicators = compute_rank_lcs(indicators)

        # -------------------------------------------------------------------------------
        def compute_rank_occ(indicators):
            indicators = indicators.copy()
            indicators = self.sort_indicators_by_metric(indicators, "OCC")
            indicators.insert(0, "rank_occ", range(1, len(indicators) + 1))
            return indicators

        indicators = compute_rank_occ(indicators)
        ...
        def check_types(indicators):
            ...
            if "global_citations" in indicators.columns:
                indicators["global_citations"] = indicators["global_citations"].astype(
                    int
                )
            if "local_citations" in indicators.columns:
                indicators["local_citations"] = indicators["local_citations"].astype(
                    int
                )
